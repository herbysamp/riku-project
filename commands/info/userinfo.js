...
module.exports = {
  ...
  aliases: ["user"],
  usage: "userinfo [@tag]",
  description: "Cari Tau Tentang Orang Yang Kamu Ingin Tau",
  run: async (client, message, args) => {
    ...
    if (!user) {
      return message.channel.send(":x: Tidak dapat menemukan orang tersebut")
    }
    ...
    if (user.user.presence.activities.length) {
      ...
      if (user.nickname !== null) embed.addField("Nama", user.nickname)
      embed.addField("Waktu Join Ke Server :", moment(user.joinedAt).format("LLLL"))
        .addField("Akun Di Buat : ", moment(user.user.createdAt).format("LLLL"))
        .addField("Informasi Umum", `ID: \`${user.user.id}\`\nDiscriminator : ${user.user.discriminator}\nBot: ${user.user.bot}\nDeleted User: ${user.deleted}`)
        .addField("Lencana Discord", newbadges.join(", ").toLowerCase() || "None")
